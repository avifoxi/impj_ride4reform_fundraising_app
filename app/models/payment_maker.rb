class PaymentMaker
	...
	def correct_amount
		if @payment_type == :registration 
			if !RideYear::OPTIONS.include?( @host_model.ride_option )
				CustomRideOption.find_by( display_name: @host_model.ride_option ).registration_fee
			else
	...
	def call_paypal
		...
		if @ppp.create_payment
			...
			@host_model.errors.add(:payment, @ppp.payment.error.message + ' ' + @ppp.payment.error.issue )
			prep_errors_hash
		end
	end
